 ...
 * <p>
 * Supports an external list of stopwords (words that will not be indexed at all), an external list
 * of exclusions (word that will not be stemmed, but indexed) and an external list of word-stem
 * pairs that overrule the algorithm (dictionary stemming). A default set of stopwords is used
 * unless an alternative list is specified, but the exclusion list is empty by default.
 * </p>
 * 
 * <a name="version"/>
 * <p>
 ...
 * {@link #DutchAnalyzer(Version, CharArraySet, CharArraySet)} also populate the default entries for
 * the stem override dictionary
 * <li>As of 3.1, Snowball stemming is done with SnowballFilter, LowerCaseFilter is used prior to
 * StopFilter, and Snowball stopwords are used by default.
 ...
 * </p>
 */
public final class DutchNormalizationAnalyzer extends Analyzer {
  ...
  /**
   * Builds an analyzer with the default stop words ({@link #getDefaultStopSet()}) and a few default
   * entries for the stem exclusion table.
   * 
   */
  public DutchNormalizationAnalyzer(Version matchVersion) {
    // historically, only this ctor populated the stem dict!!!!!
    this(matchVersion, DefaultSetHolder.DEFAULT_STOP_SET, CharArraySet.EMPTY_SET,
        DefaultSetHolder.DEFAULT_STEM_DICT);
  }

  public DutchNormalizationAnalyzer(Version matchVersion, CharArraySet stopwords) {
    // historically, this ctor never the stem dict!!!!!
    // so we populate it only for >= 3.6
    this(matchVersion, stopwords, CharArraySet.EMPTY_SET, matchVersion.onOrAfter(Version.LUCENE_36)
        ? DefaultSetHolder.DEFAULT_STEM_DICT
  ...
  public DutchNormalizationAnalyzer(Version matchVersion, CharArraySet stopwords,
    ...
    // historically, this ctor never the stem dict!!!!!
    // so we populate it only for >= 3.6
    this(matchVersion, stopwords, stemExclusionTable, matchVersion.onOrAfter(Version.LUCENE_36)
        ? DefaultSetHolder.DEFAULT_STEM_DICT
        : CharArrayMap.<String>emptyMap());
  }

  public DutchNormalizationAnalyzer(Version matchVersion, CharArraySet stopwords,
      CharArraySet stemExclusionTable, CharArrayMap<String> stemOverrideDict) {
