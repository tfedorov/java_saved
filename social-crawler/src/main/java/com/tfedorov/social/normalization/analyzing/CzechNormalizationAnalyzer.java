 ...
 * </ul>
 */
public final class CzechNormalizationAnalyzer extends StopwordAnalyzerBase {
  ...
  private static class DefaultSetHolder {
    ...
    static {
      try {
        DEFAULT_SET =
            WordlistLoader.getWordSet(IOUtils.getDecodingReader(CzechAnalyzer.class,
   ...
   */
  public CzechNormalizationAnalyzer(Version matchVersion) {
   ...
   */
  public CzechNormalizationAnalyzer(Version matchVersion, CharArraySet stopwords) {
   ...
   *         , {@link StopFilter} , and {@link CzechStemFilter} (only if version is >= LUCENE_31).
   *         If a version is >= LUCENE_31 and a stem exclusion set is provided via
   *         {@link #CzechAnalyzer(Version, CharArraySet, CharArraySet)} a
   *         {@link KeywordMarkerFilter} is added before {@link CzechStemFilter}.
   */
  @Override
  protected TokenStreamComponents createComponents(String fieldName, Reader reader) {
    ...
    if (matchVersion.onOrAfter(Version.LUCENE_31) && !this.stemExclusionTable.isEmpty()) {
