 ...
 */
public final class IrishNormalizationAnalyzer extends StopwordAnalyzerBase {
  ...
  private static final CharArraySet DEFAULT_ARTICLES =
      CharArraySet.unmodifiableSet(new CharArraySet(Version.LUCENE_CURRENT, Arrays.asList("d", "m",
          "b"), true));

  /**
   * When StandardTokenizer splits tвАСathair into {t, athair}, we don't want to cause a position
   * increment, otherwise there will be problems with phrase queries versus tAthair (which would not
   * have a gap).
   */
  private static final CharArraySet HYPHENATIONS = CharArraySet.unmodifiableSet(new CharArraySet(
      Version.LUCENE_CURRENT, Arrays.asList("h", "n", "t"), true));
   ...
   */
  public IrishNormalizationAnalyzer(Version matchVersion) {
   ...
   */
  public IrishNormalizationAnalyzer(Version matchVersion, CharArraySet stopwords) {
   ...
   *         {@link IrishLowerCaseFilter}, {@link StopFilter} , {@link KeywordMarkerFilter} if a
   *         stem exclusion set is provided and {@link SnowballFilter}.
   */
  @Override
